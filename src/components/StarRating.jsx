...
export default function StarRating({
  ...
  function setRating(next) {
    ...
    if (allowClearOnRepeat && value === next) {
      // Require a second consecutive click on the same selected value to clear.
      if (clearArmRef.current === next) {
        clearArmRef.current = null;
        onChange?.(0);
        return;
      }

      clearArmRef.current = next;
      onChange?.(next);
      return;
    }
